...
void display(vector<vector<int>> C, int m, int n)
{
    for (int i = 0; i < m; i++)
    {
        cout << "|"
...
void Strassen_algorithm(vector<vector<int>> &A, vector<vector<int>> &B, vector<vector<int>> &C, int size)
{
    ...
    else
    {
        ...
        vector<vector<int>> a(newSize, z), b(newSize, z), c(newSize, z), d(newSize, z),
...
void ConvertToSquareMat(vector<vector<int>> &A, vector<vector<int>> &B, int r1, int c1, int r2, int c2)
{
    int maxSize = max({r1, c1, r2, c2});
    int size = nextPowerOf2(maxSize);

    vector<int> z(size);
    vector<vector<int>> Aa(size, z), Bb(size, z), Cc(size, z);
